...
void nlepskim (const std::string &sample, const std::string &infile, const std::string &outfile, int nlep, bool printTriggerInfo = false)
{
    if (nlep < 1 || nlep > 3)
    {
    ...
    while ((currentFile = (TFile*)fileIter.Next()))
    {
        ...
        {
            TString ident;
            if (preg.MatchB(TString(currentFile->GetTitle())))
                ident = ((TObjString*)preg.MatchS(TString(currentFile->GetTitle()))->At(1))->GetString();
            else
                ident = "0";
            TString logFileName = "";
            logFileName.Append(TString(sample));
