...
# get site name
open(IN,"$ENV{CMS_PATH}/SITECONF/local/JobConfig/site-local-config.xml") || 
    die "faid to open $ENV{CMS_PATH}/SITECONF/local/JobConfig/site-local-config.xml\n$!\n";
my $local_stage_out = 0;
my $site;
while (my $line = <IN>){
    $line =~ s/\n//;
    $local_stage_out = 1 if ( $line =~ /\<local-stage-out\>/ );
    $local_stage_out = 0 if ( $line =~ /\<\/local-stage-out\>/ );
    $site = $1 if ( $line =~ /se-name\s+value\s*=\s*\"*(\S+?)\"*\s*\/\>/ && $local_stage_out == 1);
}
close IN;
die "failed to determine site name\n" unless defined $site;
...
while (my $line = <IN>){
    ...
    push @events, \@entry;
    my @result = split(/\n/,`python \$DBSCMD_HOME/dbsCommandLine.py --noheader -c search --query="find file where run=$entry[0] and dataset=$dataset and lumi=$entry[1] and site=$site"`);
