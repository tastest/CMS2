...
void plotElectronFRStudy(TString det, TString pt)
{
    ...
    TCut pfIso04("(pfchiso04 + TMath::Max(0.0, pfnhiso04 + pfemiso04 - ea04 * TMath::Max(0.0, rhoIsoAll)))/probe.Pt() < 0.15");
    TCut denominator("(eventSelection&(1<<4))==(1<<4) && HLT_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_probe > 0");
    TCut numerator("(leptonSelection&(1<<6))==(1<<6)"+pfIso04);
    if (det == "EE" && pt == "PT1020") {
    ...
    printf("%s %s\t Fake Rate (DetIso Denominator) = %4.3f\n", det.Data(), pt.Data(), n_numerator/n_denominator);

    float n_denominator = leptons->GetEntries(kinematics + denominator + pfIso04);
    float n_numerator = leptons->GetEntries(numerator + kinematics + denominator + pfIso04);
    printf("%s %s\t Fake Rate (PFIso Denominator) = %4.3f\n", det.Data(), pt.Data(), n_numerator/n_denominator);
