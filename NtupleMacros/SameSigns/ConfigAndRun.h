...
template <class Looper> int run (cuts_t cuts, const string &name, uint32 which_ones = 0xffffffff)
{
  ...
  Looper looper_lm0		(fLM0()		, cuts, log.c_str());	if (which_ones & (1 << LOOP_LM0   )) looper_lm0         .Loop();
...
int BaseLine ()
{
  //  return run<Looper>(samesigns_baseline_cuts, "BaseLine" );
  //  return run<Looper>(samesigns_baseline_cuts, "BaseLine", 1 << LOOP_WW );
  return run<Looper>(samesigns_baseline_cuts, "BaseLine", 1 << LOOP_TTBAR ); 
}


int MET_10 ()
{
  return run<Looper>(samesigns_met_10_cuts, "MET_10");
}

int MET_1 ()
{
  return run<Looper>(samesigns_met_1_cuts, "MET_1");
}

int SUMET_10 ()
{
  return run<Looper>(samesigns_sumet_10_cuts, "SUMET_10");
}

int SUMET_1 ()
{
  return run<Looper>(samesigns_sumet_1_cuts, "SUMET_1");
}
