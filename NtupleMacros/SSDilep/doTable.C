...
//hist file:
TFile *ftt = TFile::Open("/home/users/spadhi/public_html/processed_data_tag.root");
...
//do the ee row:
 ...
 cout << "| | DY | ttbar | Wjets | WW | WZ  | ZZ | TW | LM0 | LM1 | LM2 | LM3 | LM4 | LM5 | LM6 | LM7 | LM8 | LM9 |" << endl;

 for (int i=0; i<4; i++){

   cout << "| " << finalState[i] << " | " << DY[i] << " | " << tt[i] << " | " << wjets[i] << " | " << ww[i] << " | " << wz[i] << " | " << zz[i] << " | " << tw[i] << " | " << lm0x[i] << "|" << lm1x[i] << "|" << lm2x[i] << "|" << lm3x[i] << "|" << lm4x[i] << "|" << lm5x[i] << "|" << lm6x[i] << "|" << lm7x[i] << "|" << lm8x[i] << "|" << lm9x[i] << "|" << endl;
