...
template <class Looper> int run (cuts_t cuts, const string &name, uint32 which_ones = 0xffffffff)
{
     ...
     Looper looper_wjets	(fWjets()	, cuts, log.c_str());
	 if (which_ones & (1 << LOOP_WJETS )) looper_wjets       .Loop();
     ...
     Looper looper_tw		(ftW()		, cuts, log.c_str());
	 if (which_ones & (1 << LOOP_TW    )) looper_tw          .Loop();

     Looper looper_dyee_nf	(fDYee_nofilter()	, cuts, log.c_str());
	 if (which_ones & (1 << LOOP_DYEE_NF  )) looper_dyee_nf.Loop();
     Looper looper_dymm_nf	(fDYmm_nofilter()	, cuts, log.c_str());
	 if (which_ones & (1 << LOOP_DYMM_NF  )) looper_dymm_nf.Loop();
