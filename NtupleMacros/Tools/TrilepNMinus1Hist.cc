...
TrilepNMinus1Hist::TrilepNMinus1Hist (const Sample &s, const std::string &name, 
       ...
       h_N(s, name, bins, min, max)
{
     ...
     h_NMinus1[0] = new TrileptonHist(s, local_name, bins, min, max);
}
       ...
       h_N(s, name, bins, min, max)
{
     ...
     for (vector<string>::const_iterator i = cut_names.begin();
	  ...
	  h_NMinus1.push_back(new TrileptonHist(s, local_name, bins, min, max));
     }
}

void TrilepNMinus1Hist::Fill (cuts_t cuts_passed, enum TrileptonHypType t, double x, 
			double w)
{
