// $Id: ntupleFilter.cc,v 1.1 2010/04/13 04:11:04 fkw Exp $
...
bool select ()
{
  ...
  if( evt_run() < 132514 ) return false;
  //Beam Halo triggers
  ...
  if(trks_trk_p4().size() < 10){
    ...
    if((float)nGoodTrks/trks_trk_p4().size() >= 0.2) pass = true;
  }

  if( !pass) return false;

  for(unsigned int i = 0; i < twrs_emEt().size(); i++) {
    if( twrs_emEt().at(i) > 5.0 && fabs(twrs_eta().at(i)) < 3.0 ) {
      for( unsigned int j=0; j<twrs_emThresh().at(i).size(); j++ ) {
	if ( (twrs_emThresh().at(i).at(j)/cosh(twrs_emThreshEta().at(i).at(j))) > 5.0 ) return true;
      }
    }
  }

  for(unsigned int i = 0; i < twrsUncleaned_emEt().size(); i++) {
    if( twrsUncleaned_emEt().at(i) > 5.0 && fabs(twrsUncleaned_eta().at(i)) < 3.0 ) {
      for( unsigned int j=0; j<twrsUncleaned_emThresh().at(i).size(); j++ ) {
	if ( (twrsUncleaned_emThresh().at(i).at(j)/cosh(twrsUncleaned_emThreshEta().at(i).at(j))) > 5.0 ) return true;
      }
    }
  }
