...
bool select (bool isData)
{
  ...
  if( cms2.evt_pfmet()          > 50.0 ) return true;
  if( cms2.evt_tcmet()          > 50.0 ) return true;
  if( cms2.evt_met()            > 50.0 ) return true;
  if( cms2.evt_pfmet_type1cor() > 50.0 ) return true;

  return false;
}

void ntupleFilter_MET50(const std::string &infile, const std::string &outfile, bool printPass=false, bool isData = true, std::string runlist = "")  
{
    // set good run list
