...
DSGFakeRateLooper::DSGFakeRateLooper (Sample s, cuts_t cuts, const char *fname) 
     : DSGLooperBase(s, cuts, fname),
...
cuts_t DSGFakeRateLooper::DilepSelect (int i_hyp)
{
     // this doesn't really do anything special for now
     return DSGLooperBase::DilepSelect(i_hyp);
}

void DSGFakeRateLooper::Dilep (int i_hyp)
{
...
void DSGFakeRateLooper::FillHists (int i_hyp)
{
     ...
     if ((cuts_passed & cuts) == cuts) {
	  ...
	  const double weight = DSGLooperBase::Weight(i_hyp);
     ...
     DSGLooperBase::FillHists(i_hyp);
}

double DSGFakeRateLooper::Weight (int i_hyp)
{ 
     return Weight(i_hyp, 0);
}

double DSGFakeRateLooper::Weight (int i_hyp, int n_sig_syst)
{
     double weight = DSGLooperBase::Weight(i_hyp);
     double fr = 0;
     // this doesn't work for ee, since it assumes only one of
     // the hyp leptons is an electron
     if (abs(cms2.hyp_lt_id()[i_hyp]) == 11) {
...
double DSGFakeRateLooper::FakeSyst (enum DileptonHypType i) const
{
     switch (i) {
