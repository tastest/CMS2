...
def WriteEventCut(file,main,iabs):
    ...
    for ind in range(len(conditions)):
        file.write('  {\n')
        WriteConditions(file,main,iabs,tagName,tagIndex=ind,condition=conditions[ind])
        file.write('  }\n')
...
def WriteConditions(file,main,iabs,tagName,tagIndex,condition):

    if len(condition.parts)==0:
        WriteCutWith0Arg(file,main,iabs,tagName,tagIndex,condition)
    elif len(condition.parts)==1:
        WriteCutWith1Arg(file,main,iabs,tagName,tagIndex,condition)
    elif len(condition.parts)==2:
        WriteCutWith2Args(file,main,iabs,tagName,tagIndex,condition)
    else:
...
def WriteCutWith2Args(file,main,iabs,tagName,tagIndex,condition):
...
def WriteCutWith1Arg(file,main,iabs,tagName,tagIndex,condition):
    ...
    for item in condition.parts[0]:
        file.write('  {\n')
        WriteJobExecuteNbody(file,iabs,item,main,tagName,tagIndex,condition)
        file.write('  }\n')
