...
void AfbRecoLevel2D()
{
  ...
  for (Int_t iVar= 0; iVar < nVars; iVar++) {
    ...
    for (Int_t i= 0; i<ch_top->GetEntries(); i++) {
      ...
      if(observablename=="lep_azimuthal_asymmetry2") observable = -cos(observable);
      if ( ttmass > 0 ) {
        fillUnderOverFlow(hTop, sign(observable)*ttmass, weight, Nsolns);
        if (combineLepMinus) {
          fillUnderOverFlow(hTop, sign(observableMinus)*ttmass, weight, Nsolns);
        }
      }
    }
  ...
  // ============== Print the assymetry =============================
    ...
    pt1->Draw();
    c_test->SaveAs("2D_mtt_reco_"+acceptanceName+".pdf");
