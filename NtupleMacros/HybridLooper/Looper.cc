...
void Looper::BookHistos ()
{
	...
	for (unsigned int i = 0; i < 2; ++i)
	{
		...
		if (i == 1) det = "ee";
     		...
     		h1_pt_[i] = new TH1F(Form("%s_%s_%s_", SampleName().c_str(), "h1_pt", det.c_str()), "h1_pt", 100, 0.0, 100.0);
     		FormatHist(h1_pt_[i]);

     		h1_eta_[i] = new TH1F(Form("%s_%s_%s_", SampleName().c_str(), "h1_eta", det.c_str()), "h1_eta", 60, -3.0, 3.0);
     		FormatHist(h1_eta_[i]);

     		h1_ecalIso03_[i] = new TH1F(Form("%s_%s_%s_", SampleName().c_str(), "h1_ecalIso03", det.c_str()), "h1_ecalIso03", 100, 0.0, 1.0);
     		FormatHist(h1_ecalIso03_[i]);

     		h1_hcalIso03_[i] = new TH1F(Form("%s_%s_%s_", SampleName().c_str(), "h1_hcalIso03", det.c_str()), "h1_hcalIso03", 100, 0.0, 1.0);
     		FormatHist(h1_hcalIso03_[i]);

    		h1_tkIso03_[i] = new TH1F(Form("%s_%s_%s_", SampleName().c_str(), "h1_tkIso03", det.c_str()), "h1_tkIso03", 100, 0.0, 1.0);
     		FormatHist(h1_tkIso03_[i]);

                h1_esJuraIso03_[i] = new TH1F(Form("%s_%s_%s_", SampleName().c_str(), "h1_esJuraIso03", det.c_str()), "h1_esJuraIso03", 100, 0.0, 1.0);
                FormatHist(h1_esJuraIso03_[i]);


     		h1_wwIso_[i] = new TH1F(Form("%s_%s_%s_", SampleName().c_str(), "h1_wwIso", det.c_str()), "h1_wwIso", 100, 0.0, 1.0);
...
void Looper::FillEventHistos ()
{
	...
	if ((cuts_passed & cuts_) == cuts_) {
		...
		// min matteo tight
		if (cms2.els_tightId22XMinMatteo()[0] != 1) return;
		...
		h1_wwIso_[det]->Fill(cms2.els_p4()[0].Pt() / (isoSum + cms2.els_p4()[0].Pt()), weight);
