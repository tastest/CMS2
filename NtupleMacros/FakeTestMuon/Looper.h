...
const static cuts_t baseline_cuts = 
  //  (CUT_BIT(CUT_TRUE_MU_FROM_W_WJETS) ) | // remember to change this when using MuMu or both MuMu and EMu
  (CUT_BIT(CUT_TRUE_EL_FROM_W_WJETS) ) |
  (CUT_BIT(CUT_LT_PT)		) | 
  (CUT_BIT(CUT_LL_PT)		) | 
...
//  |  CUT_BIT(CUT_MU_GOOD) |  CUT_BIT(CUT_MU_ISO) // removed 090225 IBL
;
						 
// numerator object cuts for the fake rate prediction 
const static cuts_t fakerate_numerator_cuts = 
  fakerate_denominator_cuts | CUT_BIT(CUT_MUFAKE_NUMERATOR);
...
const static cuts_t fakerate_denominator_not_numerator_cuts = 
  fakerate_denominator_cuts | CUT_BIT(CUT_MUFAKE_NOT_NUMERATOR);
