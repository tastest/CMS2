...
void doit() {
  ...
  if( dobefore ) {
	if( doeg ) {
	  ...
	  baby_Egbefore->ScanChain(chain_Egbefore, "validate_els_before.root", "jsonlist_132440_138751.txt", true, false);
	}
	if( domu ) {
	  myBabyMaker * baby_Mubefore = new myBabyMaker();
	  TChain* chain_Mubefore = ChainFromText("Mu_before.txt");
	  baby_Mubefore->ScanChain(chain_Mubefore, "validate_mus_before.root", "jsonlist_132440_138751.txt", false, true);
	}
  }
  ...
  if( doafter ) {
	if( doeg ) {
	  ...
	  //bools are electrons, muons
	  baby_Egafter->ScanChain(chain_Egafter, "validate_els_after.root", "jsonlist_132440_138751.txt", true, false);
	}
	if( domu ) {
	  myBabyMaker * baby_Muafter = new myBabyMaker();
	  TChain* chain_Muafter = ChainFromText("Mu_after.txt");
	  baby_Muafter->ScanChain(chain_Muafter, "validate_mus_after.root", "jsonlist_132440_138751.txt", false, true);
	}
  }
