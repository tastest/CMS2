...
LeptonFakeRateLooper::LeptonFakeRateLooper (Sample s, cuts_t c, const char *fname) 
     : Looper(s, c, fname)
{
...
void LeptonFakeRateLooper::BookHistos 	()
{
...
cuts_t LeptonFakeRateLooper::DilepSelect (int i_hyp)
{
     // this doesn't do anything special
     return Looper::DilepSelect(i_hyp);
}

void LeptonFakeRateLooper::FillDilepHistos (int i_hyp)
{
...
// 	    cout<<"isFakeable(cms2.hyp_lt_index()[i_hyp]) "<<1*isFakeable(cms2.hyp_lt_index()[i_hyp])<<endl;
// 	    cout<<"isNumeratorElectron(cms2.hyp_lt_index()[i_hyp]) "<<isNumeratorElectron(cms2.hyp_lt_index()[i_hyp])<<endl;

	    if (isFakeable(cms2.hyp_lt_index()[i_hyp]) 
	      &&
	      not isNumeratorElectron(cms2.hyp_lt_index()[i_hyp]))
	      //	    cout<<"elFakeProb(cms2.hyp_lt_index()[i_hyp], 0) "<<elFakeProb(cms2.hyp_lt_index()[i_hyp], 0)<<endl;
	  ...
	  case 13:
	    if (isFakeableMuon(cms2.hyp_lt_index()[i_hyp]) 
	      ...
	      //	      cout<<"muFakeProb(cms2.hyp_lt_index()[i_hyp], 0) "<<muFakeProb(cms2.hyp_lt_index()[i_hyp], 0)<<endl;
     ...
     //     if( fr_ll!=0. || fr_lt!=0. ) cout<<"weight before"<<weight<<endl;
     weight *= fr_lt / (1 - fr_lt) + fr_ll / (1 - fr_ll);
     //     if( fr_ll!=0. || fr_lt!=0. ) cout<<"weight after"<<weight<<endl;

     // just to make sure: only one of fr_lt and fr_ll should ever be non-zero
//      if( fr_ll!=0. || fr_lt!=0. ) printf("fr_lt = %g\tfr_ll = %g\n", fr_lt, fr_ll);
...
// bool LeptonFakeRateLooper::fillErrorInPrediction(TH1F* prediction,
