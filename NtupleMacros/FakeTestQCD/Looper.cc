...
void Looper::BookHistos ()
{
  ...
  helPt_ = book1DVarHist(Form("%s_%s",sample_.name.c_str(),"elPt"),
			 Form("%s_%s",sample_.name.c_str(),"elPt"),
			 ptNBins,ptBins,
			 "p_{T}^{e} [GeV]","Events",sample_.histo_color);
  helEta_ = book1DVarHist(Form("%s_%s",sample_.name.c_str(),"elEta"),
			  Form("%s_%s",sample_.name.c_str(),"elEta"),
			  etaNBins,etaBins,
...
cuts_t Looper::EventSelect ()
{
  ...
  // check if there is at least one electron fulfilling
  // pt cut, ID cut, caloiso cut, denominator, numerator
  for ( unsigned int ele = 0;
	ele < cms2.els_p4().size();
	++ele ) {
    if (isFakeable(ele)) {
      ret |= CUT_BIT(CUT_EL_DENOMINATOR);
    }
...
void Looper::FillEventHistos ()
{
  ...
  if ((cuts_passed & cuts_) == cuts_) { 
    ...
    for ( unsigned int ele = 0; 
      ...
      // fill if ele is numerator or denominator electron
      if ( (cuts_ & CUT_BIT(CUT_EL_DENOMINATOR)) == CUT_BIT(CUT_EL_DENOMINATOR) ) {
	// prediction mode
	if ( isFakeable(ele) && !isNumeratorElectron(ele,2)) {
	  fill = true;
	}
      } else {
	// observation mode
	if ( isNumeratorElectron(ele,2)) {
	  fill = true;
	}
      }

      if ( fill == true ) {
	// and what the weight is electron dependent in the case of the denominator, numerator weight is the event weight
	const double weight = Weight(ele); 
	// if the candidate passed, we count it 
	cands_passing_ += weight; 
	cands_passing_w2_ += weight * weight; 
	cands_count_++;
	// fill histos
	helPt_->Fill(cms2.els_p4()[ele].pt(), weight);
	helEta_->Fill(cms2.els_p4()[ele].eta(), weight);
      }
