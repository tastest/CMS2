...
void get_R(HistogramUtilities *hUtil, TString hyp_type, TString nJets, Float_t &R, Float_t &err2)
{
	...
	TH1F *h1_met_in = hUtil->getHistogram(theSource, "met_in", nJets, "_" + hyp_type);
	TH1F *h1_met_out = hUtil->getHistogram(theSource, "met_out", nJets, "_" + hyp_type);
...
void get_k(TString nJets, TString hyp_type, Float_t &k, Float_t &err2)
{
    ...
    HistogramUtilities *hUtil_kMC = new HistogramUtilities("../histos_mc_3x.root", sources, 1.0);
    HistogramUtilities *hUtil_k = new HistogramUtilities("../histos_mc_3x.root", sources, 1.0);
	...
	Float_t n_mm = hUtil_k->getN(true, theSources, nJets, "mm", dummyErr2);
	Float_t n_ee = hUtil_k->getN(true, theSources, nJets, "ee", dummyErr2);
...
//	hUtil->printInOutTruth(nJets, "em");
	...
	TH1F *h1_met_in_data_em = hUtil->getHistogram(theSources, "dyest_met_in", nJets, "_em");	
	n_in_data_em = h1_met_in_data_em->Integral(metCutBin, h1_met_in_data_em->GetNbinsX() + 1);
    err2_n_in_data_em = n_in_data_em;
	//n_in_data_em = hUtil->getN(true, theSources, nJets, "em", err2_n_in_data_em);

    TH1F *h1_met_in_data_ll = hUtil->getHistogram(theSources, "dyest_met_in", nJets, "_" + hyp_type);
    n_in_data_ll = h1_met_in_data_ll->Integral(metCutBin, h1_met_in_data_ll->GetNbinsX() + 1);
    err2_n_in_data_ll = n_in_data_ll;
	//n_in_data_ll = hUtil->getN(true, theSources, nJets, hyp_type, err2_n_in_data_ll);
