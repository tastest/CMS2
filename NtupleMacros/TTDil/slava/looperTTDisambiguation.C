...
#include "looperTTDisambiguation.h"
...
int looperTTDisambiguation::ScanChain ( TChain* chain, char * prefix, float kFactor, int prescale, bool oldjets, unsigned int cutsMask){
...
// 	// bool HLTXElectronMuonRelaxed = ( (evt_HLT3() & 0x80000) != 0); 
	...
	float weight = kFactor/100; //10pb^-1
	...
	if (oldjets) {
	  ...
	  for (unsigned int ijet=0; ijet<hyp_njets().at(hypIdx); ijet++) {
...
void looperTTDisambiguation::bookHistos(char *prefix) {
