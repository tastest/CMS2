  ...
  for (int i=0; i<myNames->GetEntries(); i++) {
    ...
    for (int sample=0; sample<4; sample++) {
      ...
      hist::stack(Form("st_%s_%s",myNames->At(i)->GetName(),suffix[sample]),
		  Form("^[^_]+_%s_%s$",myNames->At(i)->GetName(), suffix[sample]), false, "",orderScheme);
      ...
      hist::stack(Form("st_ref_%s_%s",myNames->At(i)->GetName(),suffix[sample]),
		  Form("^ref_[^_]+_%s_%s$",myNames->At(i)->GetName(), suffix[sample]), false, "",orderScheme);
      THStack* refStack = (THStack*) gROOT->FindObjectAny(
