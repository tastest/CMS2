...
int ttDilCounts_looper::ScanChain (std::string fName, std::string prefix, float kFactor, int prescale, bool oldjets, unsigned int cutsMask){
  TChain* chain = new TChain("Events");
  chain->Add(fName.c_str()); //chain->GetEntries("mus_p4.pt()");
  return ScanChain(chain, prefix, kFactor, prescale, oldjets, cutsMask);
}
int ttDilCounts_looper::ScanChain (TChain* chain, std::string prefix, float kFactor, int prescale, bool oldjets, unsigned int cutsMask){
  std::vector<ProcDSChain> pds(1, ProcDSChain(chain));
  return ScanChain(pds, prefix, kFactor, prescale, oldjets, cutsMask);
}
int ttDilCounts_looper::ScanChain (std::vector<ProcDSChain>& pds, std::string prefix, float kFactor, int prescale, bool oldjets, unsigned int cutsMask){
  ...
  for(unsigned int iPDS = 0; iPDS<pds.size(); ++iPDS){
	...
	if (evId.size()%10000 == 0)std::cout<<evId.size()<<" unique events read"<<std::endl;
      }
