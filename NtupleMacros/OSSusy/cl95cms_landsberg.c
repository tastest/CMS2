...
//		if (plot) cout << "delta = " << delta << endl; 
	...
	while (fabs(delta) > epsilon) 
	{
		...
		delta = like.Integral(0,xmax,p,epsilon)/Norm - 0.95;
		if (timeout++ > 100){
		  cout << "Reached maximum number of iterations in likelihood integration: " << timeout << "; delta, xmax = " << delta << ", " << xmax << endl;
		  //if( timeout > 1000 ) cout << "timeout > 1000, exiting... " << endl; exit(0);
		}
	}         
	timeout = 0;
