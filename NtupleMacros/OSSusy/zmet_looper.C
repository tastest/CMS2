...
float returnSigma(float sumJetPt, zmet_looper::MetTypeEnum metType);
float returnBias(float sumJetPt, zmet_looper::MetTypeEnum metType);
...
void zmet_looper::makeTree(char *prefix)
{
  ...
  //variables must be declared in zmet_looper.h
...
int zmet_looper::ScanChain(TChain* chain, char *prefix, float kFactor, int prescale,
...
//           if (abs(hyp_lt_id()[hypIdx]) == 11  && (! pass_electronSelection( hyp_lt_index()[hypIdx] , electronSelection_cand01 ))) continue;
          //ttbar electron ID
          if (abs(hyp_ll_id()[hypIdx]) == 11  && (! pass_electronSelection( hyp_ll_index()[hypIdx] , electronSelection_ttbar ))) continue;
          if (abs(hyp_lt_id()[hypIdx]) == 11  && (! pass_electronSelection( hyp_lt_index()[hypIdx] , electronSelection_ttbar ))) continue;
        ...
        */
        ...
        if(!g_useBitMask){
          ...
          if (zveto == e_standard) {
            ...
            if ( ( hyp_type()[hypIdx] == 3 || hyp_type()[hypIdx] == 0 ) && hyp_p4()[hypIdx].mass() > 76. && hyp_p4()[hypIdx].mass() < 106.) continue;
          }
            ...
            if ( hyp_type()[hypIdx] == 1 || hyp_type()[hypIdx] == 2 || hyp_p4()[hypIdx].mass() < 76. || hyp_p4()[hypIdx].mass() > 106.) continue;
          }
          else{
        ...
        if(g_useBitMask){
          ...
          cut[2] = g_susybaseline ? theMet > 80. : theMet > 175.;
          ...
          if(nkcut(cutbit,ncut,2,3)){
            ...
            fillHistos(hgensumet_genmet_prof,         gen_sumEt() ,      gen_met(),           myType, nJetsIdx);
...
bool zmet_looper::passZSelection(int hypIdx)
{
...
bool zmet_looper::passTrigger(int dilType)
{
...
void zmet_looper::BookHistos(char *prefix)
{
...
float returnSigma (float sumJetPt, zmet_looper::MetTypeEnum metType)
{
  ...
  if (metType == zmet_looper::e_tcmet) {
    for( int j = 0; j < 4; j++ ) {
      if( sumJetPt < xbins[j] )
  ...
  else if (metType == zmet_looper::e_muonjes) {
    for( int j = 0; j < 4; j++ ) {
      if( sumJetPt < xbins[j] )
