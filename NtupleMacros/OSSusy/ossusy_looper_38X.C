...
float returnSigma(float sumJetPt, ossusy_looper_38X::MetTypeEnum metType);
float returnBias(float sumJetPt, ossusy_looper_38X::MetTypeEnum metType);
...
void ossusy_looper_38X::makeTree(char *prefix)
{
  ...
  //variables must be declared in ossusy_looper_38X.h
...
void ossusy_looper_38X::closeTree()
{
  outFile->cd();
  outTree->Write();
  outFile->Close();
...
ossusy_looper_38X::ossusy_looper_38X()
{
...
bool ossusy_looper_38X::isFakeableMuon (int index) {
...
//get the FR weight
// *****************************************************************
double ossusy_looper_38X::getFRWeight(const int hypIdx, string elFRversion, SimpleFakeRate* mufr, SimpleFakeRate * elfr, FREnum frmode, bool isData) {

  //std::cout<<"Called ossusy_looper_38X::getFRWeight"<<std::endl;
  ...
  if ( frmode == e_qcd ) {
    ...
    std::cout<<"ossusy_looper_38X::getFRWeight: bad FR mode given, fix this!"<<std::endl;
    return -9999.;
  }
...
int ossusy_looper_38X::ScanChain(TChain* chain, char *prefix, float kFactor, int prescale, float lumi,
...
bool ossusy_looper_38X::passZSelection(int hypIdx)
{
...
bool ossusy_looper_38X::passTrigger(int dilType)
{
...
void ossusy_looper_38X::BookHistos(char *prefix)
{
...
float ossusy_looper_38X::smearMet( float met , float sumjetpt , float metscale ){
...
float returnSigma (float sumJetPt, ossusy_looper_38X::MetTypeEnum metType)
{
  ...
  if (metType == ossusy_looper_38X::e_tcmet) {
    for( int j = 0; j < 4; j++ ) {
      if( sumJetPt < xbins[j] )
  ...
  else if (metType == ossusy_looper_38X::e_muonjes) {
    for( int j = 0; j < 4; j++ ) {
      if( sumJetPt < xbins[j] )
...
float ossusy_looper_38X::getCosThetaStarWeight(){
