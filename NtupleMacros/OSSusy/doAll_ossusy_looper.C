...
void doAll_ossusy_looper(bool skipFWLite = true)
{
    ...
    for (int jetTypeIdx = 1; jetTypeIdx < 2; ++jetTypeIdx)
    {
        ...
        {
            for (int zvetoIdx = 0; zvetoIdx < 3; ++zvetoIdx)
            {
                ...
                if(doFakeApp) {
                    const char* outFile = Form("victory_baseline_metgt50_sumjetptgt200_cand01_%s_%s%s_FakeApp_3x.root", 
                            jetTypeStrings[jetTypeIdx], metTypeStrings[metTypeIdx],zvetoStrings[zvetoIdx]);
                }
                else {
                    const char* outFile = Form("victory_baseline_metgt50_sumjetptgt200_cand01_%s_%s%s_3x.root", 
                            jetTypeStrings[jetTypeIdx], metTypeStrings[metTypeIdx],zvetoStrings[zvetoIdx]);
                }
