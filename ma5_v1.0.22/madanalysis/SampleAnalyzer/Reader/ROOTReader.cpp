...
bool ROOTReader::ReadHeader(SampleFormat& mySample)
{
  ...
  if (!source_->IsOpen()) 
  {
    ERROR << "ROOT file called '" << mySample.name() << "' is not found" << std::endl;
    return false;
  }
  ...
  if (file_version!=lib_version)
  {
    WARNING << "the input file has been produced with ROOT version " << file_version
            << " whereas the loaded ROOT libs are related to the version " << lib_version << std::endl;
  }
  ...
  if (tree_==0)
  {
    ERROR << "the ROOT TTree 'tree' is not found in the file" << std::endl;
    return false;
  }
  ...
  INFO << "ROOTdebug nevents in ttree : " << nevents_ << std::endl;
  ncurr_=0;
  return true;
}
...
// ReadEvent
// -----------------------------------------------------------------------------
bool ROOTReader::ReadEvent(EventFormat& myEvent, SampleFormat& mySample)
{
  // Initiliaze MC
  myEvent.InitializeRec();
  myEvent.InitializeMC();

  if (ncurr_ >= nevents_) return false;
...
void ROOTReader::FillEvent(EventFormat& myEvent, SampleFormat& mySample)
{
  ...
  // MET
  myEvent.rec()->MET_.met_.Set(evt_->met.px(),evt_->met.py());
}
